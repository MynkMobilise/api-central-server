...
import panImage from './panImages/pan_images.jpg';

export default function PanOCR() {
  const pageTitle = "Pan Ocr";
  ...
  const runAnalyzer = () => {
    ...
    OcrServices.postPanImgDetails(params).then((res) => {
      console.log(res);
      setTimeout(() => {
                    ...
                    <div className="card border-0 rounded-0 h-100">
                      ...
                      <div className={`card-body p-3 ${Texttospeech["bck-clr"]}`}>
                        ...
                        <div className={`tab-content h-60vh ${Texttospeech['tab']}`} id="myTabContent">
                            ...
                            <tbody>
                            {outputJson && outputJson[0]?.result && outputJson[0]?.result.extraction_output ? (
                                Object.entries(outputJson[0]?.result.extraction_output).map(([key, value]) => (
                                    <tr key={key}>
                                    <td>{key}</td>
                                    <td>{value}</td>
                                    </tr>
                            ...
                            <div
                              ...
                              <textarea
                            className="text-dark form-control" rows={10}
