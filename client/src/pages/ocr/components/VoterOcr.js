...
import panImage from './voterImage/voter_card.jpeg';

export const VoterOcr = () => {
    const pageTitle = "Voter Card OCR";

    // const [idCardArr, setIdCardArr] = useState([]);
    // const [jsonArr, setJsonArr] = useState([]);
    const [outputJson, setJsonOutput] = useState([]);
    ...
    const runAnalyzer = () => {
        ...
        OcrServices.getVoterCardOcr(params).then((res) => {
            console.log(res);
            setJsonOutput(res);
            setLoader(false);
        });
