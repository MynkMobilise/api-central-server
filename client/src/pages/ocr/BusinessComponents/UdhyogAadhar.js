...
export default function UdhyogAadhar() {
  ...
  function validateForm() {
    const newErrors = {};
    if (!panForm.uam_number) {
      newErrors.uam_number = "CIN no is required.";
    }
  ...
  const runSamples = () => {
    panForm.uam_number = 'L27100MH1907PLC000260';
  ...
  function submit(event) {
    ...
    const params = {
      ...
      data: {
        uam_number: panForm.uam_number,
      },
    };

    OcrServices.postUdhyogAdarDetails(params).then(
      (res) => {
        console.log(res);
        setTimeout(() => {
