...
import gen_card1 from '../DocumentSamples/gen_doc.pdf';
const { DocumentAnalysisClient } = require("@azure/ai-form-recognizer");

export default function GeneralDocument() {
  // Define Varibles Here

  const pageTitle = "General Document Ocr";
  ...
  const handleSample = async  (id) => {
    ...
    if(id == 1)
    {
      const response = await fetch(gen_card1);
  ...
  async function startAnalyzingDoc(img) {
    ...
    const poller = await client.beginAnalyzeDocument('prebuilt-document',img) ;
                            ...
                            <div className='file file--disabled'>
                                ...
                                {uploadImage == '' ? (
                                    <i className="fas fa-file-pdf display-1"></i>
                                      ...
                                      <i className="fas fa-file-pdf text-dark display-1"></i>
                                )}
                                <p>
                                <small>
                                Upload File
