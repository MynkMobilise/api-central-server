...
import b_card1 from '../DocumentSamples/business_card.jpg';
import b_card2 from '../DocumentSamples/b_card2.webp';
import b_card3 from '../DocumentSamples/b_card3.webp';
const { DocumentAnalysisClient } = require("@azure/ai-form-recognizer");

export default function BusinessCard() {
  // Define Varibles Here

  const pageTitle = "Business Card Ocr";
  ...
  const handleSample = async  (id) => {
    ...
    if(id == 1)
    {
      const response = await fetch(b_card1);
    ...
    {
      const response = await fetch(b_card2);
      const pdfData = await response.blob();
    ...
    {
      const response = await fetch(b_card3);
      const pdfData = await response.blob();
  ...
  async function startAnalyzingDoc(img) {
    ...
    const poller = await client.beginAnalyzeDocument('prebuilt-businessCard',img) ;
     ...
     if (result) {
       ...
       if (result.docType) {
           ...
           for (const key in result.fields) {
              //  data.push(`${key}: ${result.fields[key].values[0].content}`)
               data.push({key, value: result?.fields[key]?.content})
               
               console.log(`${key}: ${result.fields[key].values[0].content}`);
           }
