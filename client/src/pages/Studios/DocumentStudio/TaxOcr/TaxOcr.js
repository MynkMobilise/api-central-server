...
import tax_img from '../DocumentSamples/w2.png';
const { DocumentAnalysisClient } = require("@azure/ai-form-recognizer");

export default function TaxOcr() {
  // Define Varibles Here

  const pageTitle = "US Tax Ocr W-2";
  ...
  const handleSample = async  (id) => {
    ...
    if(id == 1)
    {
      const response = await fetch(tax_img);
  ...
  async function startAnalyzingDoc(img) {
    ...
    const poller = await client.beginAnalyzeDocument('prebuilt-tax.us.w2',img) ;
