...
import id_Card1 from '../DocumentSamples/identity_documents.png';
import id_Card2 from '../DocumentSamples/id_card.jpg';
import id_Card3 from '../DocumentSamples/id_card2.webp';
// import id_Card4 from '../DocumentSamples/id_Card3.avif';
...
export default function IdCardOcr() {
  ...
  const pageTitle = "Identification Card Ocr";
  ...
  const handleSample = async  (id) => {
    ...
    if(id == 1)
    {
      const response = await fetch(id_Card1);
    ...
    {
      const response = await fetch(id_Card2);
      const pdfData = await response.blob();
    ...
    {
      const response = await fetch(id_Card3);
      const pdfData = await response.blob();
    ...
    // else if(id == 4)
    // {
    //   const response = await fetch(id_Card4);
    //   const pdfData = await response.blob();
  ...
  async function startAnalyzingDoc(img) {
    ...
    const poller = await client.beginAnalyzeDocument('prebuilt-idDocument',img) ;
