...
import readOcr1 from '../DocumentSamples/readOcr.png';
import readOcr2 from '../DocumentSamples/readOcr2.jpg';
import readOcr3 from '../DocumentSamples/readOcr3.webp';
const { DocumentAnalysisClient } = require("@azure/ai-form-recognizer");

export default function ReadOcr() {
  // Define Varibles Here

  const pageTitle = "Read Page Ocr";
  ...
  const handleSample = async  (id) => {
    ...
    if(id == 1)
    {
      const response = await fetch(readOcr1);
    ...
    {
      const response = await fetch(readOcr2);
      const pdfData = await response.blob();
    ...
    {
      const response = await fetch(readOcr3);
      const pdfData = await response.blob();
  ...
  const runAnalyzer = () => {
    ...
    if(!uploadImageTemp)
    {
      alert("Please Upload Any Artical Image");
      setLoader(false)
    }
    else{
                ...
                <div className={`col-md-6 pr-0 ${Texttospeech["bdr-btw"]}`}>
                  <div className="card border-0 rounded-0">
                    ...
                    >
                      ...
                      <div className={`${Texttospeech['direction']} mb-3`}>
