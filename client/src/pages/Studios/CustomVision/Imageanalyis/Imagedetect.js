...
export const Imagedetect = () => {
  ...
  const pageTitle = "Detect Common Objects From Image";
  ...
  const describeImage = async () => {
    ...
    try {
      const result = await computerVisionClient.analyzeImageInStream(uint8Array, { visualFeatures: ['Objects', 'Description'] });
      // tags, objects, caption, denseCaptions, read, smartCrops, people
      // "ImageType","Faces","Adult","Categories","Color","Tags","Description","Objects","Brands"
  ...
  const countSmiles = () => {
    if (analysis && analysis.tags && analysis.objects) {
      const tagsWithSmile = analysis.tags.filter(tag => tag.name.toLowerCase() === "smile");
      const objectsWithSmile = analysis.objects.filter(obj => obj.object.toLowerCase() === "smile");

      const totalSmileCount = tagsWithSmile.length + objectsWithSmile.length;
      setSmileCount(totalSmileCount);
    }
  };
      ...
      <div className={`container-fluid ${Texttospeech["text-wrapper"]}`}>
        ...
        <div className="card border-0 bg-light">
          ...
          <div className={`${Texttospeech["bdr"]}`}>
            ...
            >
              Detect sensitive content in images so you can moderate their usage in your applications. You can use this feature to detect when images contain Adult content (explicitly sexual in nature, often showing nudity and sexual acts), Racy content (sexually suggestive in nature, often containing less sexually explicit) or Gory content (images showing blood/gore).
            </p>
                            ...
                            <div className="tab-pane fade overflow-scroll h-75 show active p-3" id="table" role="tabpanel" aria-labelledby="table-tab">
                              {analysis && analysis.objects && (
                                <table className='table table-bordered table-striped'>
                                  ...
                                  <tbody>
                                    {analysis && analysis.objects && analysis.objects.map((obj, index) => (
                                      <tr key={index}>
                                        {obj.object}
                                      </tr>
                                    ))}
                              ...
                              {analysis && analysis.objects && (
                                <table className='table table-bordered table-striped'>
                                  <thead>
                                    <th>Total Peoples in Image</th>
                                  </thead>
                                  <tbody>
                                    <tr>{analysis.objects.filter(obj => obj.object === "person").length}
                                    </tr>
                                  </tbody>
                                </table>
                              )}
